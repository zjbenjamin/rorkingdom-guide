...
function parseTextToRichContent(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // 过滤掉可能由admin.js自动生成的重复文本
    if (cleanLine.indexOf('🎵 推荐单曲:') > -1 || cleanLine.indexOf('📺 视频:') > -1 || cleanLine.indexOf('🎬 视频:') > -1 || cleanLine.indexOf('口推荐单曲:') > -1) {
      continue;
    }
    ...
    if (urlMatch) {
      ...
      var isMusic = /music\.163\.com|163cn\.tv|y\.qq\.com/.test(url) || cleanLine.indexOf('单曲') > -1 || cleanLine.indexOf('音乐') > -1 || cleanLine.indexOf('生日祝福歌') > -1;
      ...
      if (isMusic || isVideo) {
        ...
        if (isMusic) {
          ...
          if (/bilibili\.com/.test(url)) { platform = 'bilibili'; platformName = 'B站'; }
    ...
    if (cleanLine.indexOf('视频：') === 0 || cleanLine.indexOf('口推荐单曲:') === 0) {
      ...
      db.collection('announcements').where({ type: 'event' }).orderBy('createTime', 'desc').limit(50).get()
        .then(function(res) {
          var cloudActivities = (res.data || []).map(function(item) {
            ...
            return { id: item._id, title: item.title, desc: item.content, status: item.pinned ? '置顶' : '进行中', type: item.type || '', subType: item.subType || 'regular', collabLogo: item.collabLogo || '', rewards: item.rewards || [], start: item.start || '', end: item.end || '', image: item.image || '', richContent: richContent, isCloud: true }
          })
