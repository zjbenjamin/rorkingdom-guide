...
Page({
  ...
  loadComments: function(refresh) {
    ...
    query.skip(page * self.data.pageSize)
      ...
      query = query.where({ userName: app.globalData.userInfo.nickName })
      query = query.orderBy('createTime', 'desc')
    } else {
      query = query.orderBy('createTime', 'desc')
    }
    query.skip(page * self.data.pageSize)
      ...
      .then(function(res) {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          var d = new Date(list[i].createTime)
          var dayKey = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate()
          if (!daySet[dayKey] && list[i].replyTo === null) {
            list[i].isFirstToday = true
            daySet[dayKey] = true
          }
        }
        ...
        var imageRefs = []
        for (var p = 0; p < comments.length; p++) {
          if (cloudUrl.isCloudUrl(comments[p].userAvatar)) {
            allFileIDs.push(comments[p].userAvatar)
            avatarMap[comments[p].userAvatar] = true
          }
          if (comments[p].images) {
            for (var q = 0; q < comments[p].images.length; q++) {
        ...
        var uniqueIDs = []
        var seen = {}
        for (var r = 0; r < allFileIDs.length; r++) {
          if (!seen[allFileIDs[r]]) { seen[allFileIDs[r]] = true; uniqueIDs.push(allFileIDs[r]) }
        }
        if (uniqueIDs.length > 0) {
          wx.cloud.getTempFileURL({ fileList: uniqueIDs }).then(function(urlRes) {
            var urlMap = {}
            if (urlRes.fileList) {
              for (var s = 0; s < urlRes.fileList.length; s++) {
                if (urlRes.fileList[s].tempFileURL) urlMap[urlRes.fileList[s].fileID] = urlRes.fileList[s].tempFileURL
              }
            }
